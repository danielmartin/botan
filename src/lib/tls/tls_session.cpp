...
const char* TLS_SESSION_HMAC = "HMAC(SHA-512-256)";
// SIV would be better but can't assume it is available
const char* TLS_SESSION_AEAD = "AES-256/GCM";
const char* TLS_SESSION_KEY_NAME = "BOTAN TLS SESSION KEY NAME";
const size_t TLS_SESSION_KEY_NAME_LEN = 4;
const size_t TLS_SESSION_AEAD_NONCE_LEN = 12;
const size_t TLS_SESSION_AEAD_KEY_SEED_LEN = 16;
const size_t TLS_SESSION_AEAD_TAG_SIZE = 16;
...
Session::encrypt(const SymmetricKey& key, RandomNumberGenerator& rng) const
   {
   auto hmac = MessageAuthenticationCode::create_or_throw(TLS_SESSION_HMAC);
   hmac->set_key(key);

   // First derive the "key name"
   std::vector<uint8_t> key_name(hmac->output_length());
   hmac->update(TLS_SESSION_KEY_NAME);
   hmac->final(key_name.data());
   key_name.resize(TLS_SESSION_KEY_NAME_LEN);

   std::vector<uint8_t> aead_nonce;
   std::vector<uint8_t> key_seed;

   rng.random_vec(aead_nonce, TLS_SESSION_AEAD_NONCE_LEN);
   rng.random_vec(key_seed, TLS_SESSION_AEAD_KEY_SEED_LEN);
   ...
   std::unique_ptr<AEAD_Mode> aead = AEAD_Mode::create_or_throw(TLS_SESSION_AEAD, ENCRYPTION);
   BOTAN_ASSERT_NOMSG(aead->valid_nonce_length(TLS_SESSION_AEAD_NONCE_LEN));
   BOTAN_ASSERT_NOMSG(aead->tag_size() == TLS_SESSION_AEAD_TAG_SIZE);
...
Session Session::decrypt(const uint8_t in[], size_t in_len, const SymmetricKey& key)
   ...
   try
      {
      const size_t hdr_len = TLS_SESSION_KEY_NAME_LEN + TLS_SESSION_AEAD_NONCE_LEN +
         TLS_SESSION_AEAD_KEY_SEED_LEN;
      const size_t min_len = hdr_len + TLS_SESSION_AEAD_TAG_SIZE;

      if(in_len < min_len)
         throw Decoding_Error("Encrypted session too short to be valid");

      const uint8_t* key_name = &in[0];
      const uint8_t* key_seed = key_name + TLS_SESSION_KEY_NAME_LEN;
      const uint8_t* aead_nonce = key_seed + TLS_SESSION_AEAD_KEY_SEED_LEN;
      const uint8_t* ctext = aead_nonce + TLS_SESSION_AEAD_NONCE_LEN;
      const size_t ctext_len = in_len - hdr_len; // includes the tag
      ...
      if(same_mem(cmp_key_name.data(), key_name, TLS_SESSION_KEY_NAME_LEN) == false)
         throw Decoding_Error("Wrong key name for encrypted session");

      hmac->update(key_seed, TLS_SESSION_AEAD_KEY_SEED_LEN);
      const secure_vector<uint8_t> aead_key = hmac->final();

      auto aead = AEAD_Mode::create_or_throw(TLS_SESSION_AEAD, DECRYPTION);
      aead->set_key(aead_key);
      aead->set_associated_data(in, hdr_len);
      aead->start(aead_nonce, TLS_SESSION_AEAD_NONCE_LEN);
